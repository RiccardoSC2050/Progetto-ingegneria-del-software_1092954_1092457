...
/**
 * Abstract base class for all system operators.
 * Provides common functionality for user authentication, access control, and operator management.
 * 
 * Access Levels:
...
abstract class Operator {
	...
	/**
	 * Constructor for creating regular users with specified access level.
	 * Used by Root to create new users. Validates access level range (0-3).
	 * Automatically generates a unique 8-character ID and adds the operator to the global list.
	 * 
	 * @param name the username for the operator
	 * @param password the password for authentication
	 * @param accessLevel the permission level (0-3 for regular users)
	 * @throws InvalidAccessLevelException if accessLevel is outside valid range (0-3)
	 */
	...
	/**
	 * Constructor for creating the Root operator.
	 * Creates the administrator with fixed ID "0" and access level 5.
	 * Used exclusively by the Root class for singleton pattern implementation.
	 * 
	...
	/**
	 * Authenticates an operator by comparing credentials against all registered operators.
	 * Searches through the global operator list to find matching name and password.
	 * 
	...
	/**
	 * Logs out the current operator from the system.
	 * Displays a disconnection message and performs cleanup operations.
	 * 
	 ...
	 * Includes name, access level, and unique identifier.
	 * 
	...
	/**
	 * Returns the complete list of all registered operators.
	 * Provides access to the global operator registry.
	 * 
	...
	/**
	 * Removes a specific operator from the global operator list.
	 * Performs safety checks to prevent manipulation of empty lists.
	 * 
	...
	/**
	 * Sets a new password for the operator.
	 * Updates the operator's authentication credentials.
	 * 
	...
	/**
	 * Gets the operator's password for authentication purposes.
	 * Access restricted to package level to prevent unauthorized password exposure.
	 * Should only be used when the caller has the operator's UUID for verification.
	 * 
